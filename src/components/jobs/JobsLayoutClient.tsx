...
// Navigation groups - same as dashboard
...
export function JobsLayoutClient({ children }: JobsLayoutClientProps) {
  ...
  // Fetch badge counts for logged-in users
  useEffect(() => {
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-3">
              <Link
                href="/auth/employee/login?redirect=/dashboard"
                onClick={() => setShowLoginModal(false)}
      ...
      >
        <div className="flex flex-col h-full">
                ...
                <div className="space-y-1">
                  {group.items.map((item) => {
                    ...
                    return (
                      <Link
                        key={item.href}
                        href={isLoggedIn || item.href === '/jobs' ? item.href : '#'}
          ...
          <div className="p-3 border-t border-slate-100">
            {/* Profile Completion Progress - only for logged in users */}
            {isLoggedIn && profileCompletion < 100 && (
